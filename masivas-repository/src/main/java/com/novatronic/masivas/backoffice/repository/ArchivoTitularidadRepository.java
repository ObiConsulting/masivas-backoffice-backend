...
public interface ArchivoTitularidadRepository extends JpaRepository<TpArchivoTitularidad, Long> {
    ...
    @Query("    SELECT NEW com.novatronic.masivas.backoffice.dto.DetalleConsultaArchivoTitularidadDTO("
            + "t.idArchivo, t.nombre, t.fechaObtencion, t.fechaProcesada, t.cantidadDeclaradoCliente ,"
            + " t.cantidadProcesado, "
            + "    CAST(CASE "
            + "        WHEN t.estadoEnviadoCliente IN (600L,601L) THEN (SELECT p.valor FROM TpParametro p WHERE p.codigo = '0703') "
            + "        WHEN t.estadoObtenidoCCE =400L THEN (SELECT p.valor FROM TpParametro p WHERE p.codigo = '0702') "
            + "        WHEN t.estadoEnviadoCCE =300L THEN (SELECT p.valor FROM TpParametro p WHERE p.codigo = '0701') "
            + "        WHEN t.estadoObtenidoCliente IN (100L,101L,102L) OR t.estadoProcesadoCliente IS NOT NULL THEN (SELECT p.valor FROM TpParametro p WHERE p.codigo = '0700') "
            + "        ELSE 'Pendiente' "
            + "    END AS string), "
            + "    CASE "
            ...
            + "     WHERE (t.fechaObtencion IS NULL OR t.fechaObtencion >= :fechaInicioObtencion)\n"
            + "      AND (t.fechaObtencion IS NULL OR t.fechaObtencion <= :fechaFinObtencion)\n"
            + "      AND (t.fechaProcesada IS NULL OR t.fechaProcesada >= :fechaInicioProcesada)\n"
            + "      AND (t.fechaProcesada IS NULL OR t.fechaProcesada <= :fechaFinProcesada)\n"
            + "      AND (:estado IS NULL OR "
            + "         CAST(CASE "
            + "            WHEN t.estadoEnviadoCliente IN (600L,601L) THEN '0703' "
            + "            WHEN t.estadoObtenidoCCE =400L THEN '0702' "
            + "            WHEN t.estadoEnviadoCCE =300L THEN '0701' "
            + "            WHEN t.estadoObtenidoCliente IN (100L,101L,102L) THEN '0700' "
            + "            ELSE 'Pendiente' "
    ...
    @Query("SELECT "
            + "CAST(CASE "
            + "    WHEN t.estadoEnviadoCliente IN (600L,601L)  THEN 'Enviado Cliente' "
            + "    WHEN t.estadoProcesadoCCE IS NOT NULL THEN 'Obtenido CCE' "
            + "    WHEN t.estadoObtenidoCCE =400L THEN 'Obtenido CCE' "
            + "    WHEN t.estadoEnviadoCCE =300L  THEN 'Enviado CCE' "
            + "    WHEN t.estadoProcesadoCliente IS NOT NULL THEN 'Obtenido Cliente' "
            + "    WHEN t.estadoObtenidoCliente IN (100L,101L,102L)  THEN 'Obtenido Cliente' "
            + "    ELSE 'Pendiente' "
            + "END AS string), "
            + "COUNT(t.idArchivo) "
            + "FROM TpArchivoTitularidad t "
